...
RUN dnf update -y &&\
    dnf install -y bison clang cmake cubeb-devel findutils freeglut-devel git glm-devel gtk3-devel kernel-headers libgcrypt-devel libsecret-devel libtool libusb1-devel llvm nasm ninja-build perl-open perl-FindBin perl-File-Compare perl-File-Copy perl-IPC-Cmd python3-jinja2 python3-passlib rsync systemd-devel vcpkg vim-enhanced wayland-protocols-devel wget zip zlib-devel &&\
